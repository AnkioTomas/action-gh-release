...
export const releaseBody = (config: Config): string | undefined => {
  return (config.input_body_path && readFileSync(config.input_body_path).toString('utf8')) || config.input_body
}
...
export const unmatchedPatterns = (patterns: string[]): string[] => {
  return patterns.reduce((acc: string[], pattern: string): string[] => {
    return acc.concat(glob.sync(pattern).filter(path => statSync(path).isFile()).length === 0 ? [pattern] : [])
  }, [])
}
